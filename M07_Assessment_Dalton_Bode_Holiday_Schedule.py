...
class HolidayList():
    ...
    def addHoliday(self, holidayName, date):
        ...
        if type(Holiday(holidayName, date)) == type(Holiday()):
            self.innerHolidays.append(Holiday(holidayName, date).__dict__)

    def findHoliday(self, holidayName, date):
        
        # Find Holiday in innerHolidays
        if Holiday(holidayName, date) in self.innerHolidays:
            return str(Holiday(holidayName, date))
        else:
            print(f"{holidayName} ({date}) is not in the holiday list.")

    def removeHoliday(self, holidayName, date):
        # Removes the holiday object from the list
        if type(Holiday(holidayName, date)) == type(Holiday()):
            self.innerHolidays.pop(self.innerHolidays.index(Holiday(holidayName, date)))
    ...
    def save_to_json(self, fileName):
        ...
        with open(fileName, "wt", encoding = 'utf-8') as holidayList:
            json.dump(self.innerHolidays, holidayList)
        ...
        for year in yearList:
            ...
            try:
                for row in soup.find('table', attrs = {'class':"cht lpad"}).find_all('tr'):
                    ...
                    if Holiday(row.find_all('td')[1].text, datetime.date(year,holidayMonth,day)) not in self.innerHolidays:
...
#         def weekNumCreator(holiday):
#             dateobj = datetime.date(int(str(holiday['date'])[0:4]),int(str(holiday['date'])[5:7]),int(str(holiday['date'])[8:10]))                                               
#             return {'name':holiday['name'],'date':str(dateobj),'weekNum':dateobj.isocalendar()[1]}               
                                                             
#         holidayWeekNumbers = list(map(lambda x: weekNumCreator(x), self.innerHolidays))
        
        # Used as a function to be mapped to each element of the filtered list to retrieve holiday information
...
def add_Holiday(holidayList):
    ...
    while True:
        ...
        try:
            ...
            if Holiday(addHol,addNewDate) not in holidayList.innerHolidays:
...
def remove_Holiday(holidayList):
    ...
    while True:
        ...
        try:
            ...
            if Holiday(remHol,remDate) in holidayList.innerHolidays:
